...
const ReviewProductArea = () => {
  ...
  const [countOfPage, setCountOfPage] = useState<number>(5);
  ...
  if (!isError && reviewProducts?.success) {
    let review_items = reviewProducts.data;
    const totalPage = Math.ceil(review_items.length / countOfPage);
      ...
      <>
        ...
        <div className="relative overflow-x-auto  mx-8">
          <table className="w-[1400px] 2xl:w-full text-base text-left text-gray-500">
            ...
            <tbody>
              {review_items.slice(pageStart, pageStart + countOfPage).map((item) => (
        ...
        {review_items.length > countOfPage && (
          <div className="flex justify-between items-center flex-wrap mx-8">
            <p className="mb-0 text-tiny mr-3">
              Showing 1-
              {review_items?.slice(pageStart, pageStart + countOfPage).length} of {review_items.length}
            </p>
